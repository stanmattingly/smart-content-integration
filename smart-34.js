            ...
            const observer = new MutationObserver(mutations_list => {
                mutations_list.forEach(mutation => {
                    mutation.removedNodes.forEach(removedNode => {
                        let xpath = createXPathFromElement(removedNode);
                        if (xpath && xPaths.includes(xpath)) {
                            unprocessedXPaths.push(xpath)
                        } else {
                            xPaths.forEach(xpathString => {
                                let element = getElementByXpath(xpathString, removedNode);

                                if (element) {
                                    unprocessedXPaths.push(xpathString);
                                }
                            })
                        }
                    })
