            ...
            response.json().then(() => {
                document.addEventListener('click', function (event) {
                    ...
                    if (event.target.href) {
                        event.target.href = `${event.target.href}?smart-add-token=${smartAddToken}`;
                    }

                    if (event.target.parentElement.href) {
                        event.target.parentElement.href = `${event.target.parentElement.href}?smart-add-token=${smartAddToken}`;
                    }
                }, true);
