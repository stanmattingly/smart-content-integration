    ...
    fetch(`https://6767-172-98-142-66.ngrok.io/auth/smart-add-tokens/${smartAddToken}/`, {
        ...
        if (response.ok) {
                        ...
                        if (!element.dataset.clicked) {
                            ...
                            fetch("https://6767-172-98-142-66.ngrok.io/api/app/components/", {
                                ...
                                body: JSON.stringify({
                                    ...
                                    initial_content: event.target.textContent,
            ...
            function addElementListeners(element, component) {
                for (let i in events) {
                    if (element.parentElement.tagName === 'BUTTON' || element.parentElement.tagName === 'A') {
                        element.parentElement.dataset.smartContentId = element.smartContentId;
            ...
            const observer = new MutationObserver(mutations_list => {
                mutations_list.forEach(mutation => {
                    mutation.addedNodes.forEach(addedNode => {
                        const xpath = createXPathFromElement(addedNode);
